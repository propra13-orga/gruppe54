...
public class Falle extends Rectangle {
	...
	public static void bewegung(){
		...
		if (Spielfeld.getBlockID(Falle.StartX+16, Falle.StartY+32)!=1){
       	 Falle.StartY+=1;
       	 Kollision();
       	 } else 
       		 if (Spielfeld.getBlockID(Falle.StartX+16, Falle.StartY+32)==1){
	...
	public static void Kollision(){
		if( (Falle.StartX+31 >= Spielfeld.spieler.x)     && 
				(Falle.StartX <= Spielfeld.spieler.x+31)  &&
				(Falle.StartY+31 >= Spielfeld.spieler.y)  &&
